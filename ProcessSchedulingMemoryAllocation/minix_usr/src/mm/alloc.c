...
{
	phys_clicks buffer[BUFF_SIZE];
	register struct hole *hp = hole_head;
	int i = 0;
	if(hp == NIL_HOLE) return 0;
	while(hp != NIL_HOLE && hp->h_base < swap_base && i < BUFF_SIZE - 2)
	{
		buffer[i++] = hp->h_len;
		buffer[i++] = hp->h_base;
		hp = hp->h_next;
 ...
 */
  ...
  register struct hole *hp, *prev_ptr, *max, *prev_max;
  ...
  else
  {
  	...
  	do
  	{
  		...
  		while(hp != NIL_HOLE && hp->h_base < swap_base)
  		{
  			if(hp->h_len >= clicks && max->h_len < hp->h_len)
  			{
  				prev_max = hp;
  				max = hp;
  			}
  		...
  		if(hp == NIL_HOLE && max->h_len >= clicks)
  		{
  			old_base = max->h_base;
			max->h_base += clicks;
			max->h_len -= clicks;
