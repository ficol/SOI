...
struct proc *find_user_proc(int pid)
{
	int i;
...
{
	struct proc *rp;
	if( (rp = find_user_proc(m_ptr->m1_i1) ) != NIL_PROC)
		return rp->group;
	return EINVAL;
}
...
message *m_ptr;
{
	struct proc *rp;
	unsigned char a = 0;
	unsigned char new_group = m_ptr->m2_l2;
	if( (rp = find_user_proc(m_ptr->m1_i1) ) == NIL_PROC)
		return EINVAL;
	if( new_group != 0 && new_group != 1)
		return EINVAL;
	rp->group = new_group;
	return 1;
}
