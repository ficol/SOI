...
#include <minix/com.h>
#include "proc.h"
unsigned char group_time[2] = { 0, 1 };
 ...
 */
  unsigned char time = group_time[DEFAULT_GROUP];

  if (rdy_head[USER_Q] == NIL_PROC) return;
  rdy_head[USER_Q]->counter+=1;
  if(rdy_head[USER_Q]->group == GROUP_B) time = group_time[GROUP_B];
  if(rdy_head[USER_Q]->counter <= time)
  {
  	pick_proc();
  	return;
  }
  rdy_head[USER_Q]->counter = 0;
